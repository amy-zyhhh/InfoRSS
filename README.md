...
- `content/raw/daily/`：按日期生成当天原文目录页，归档页面会链接到这里。

这些页面展示的是“已抓取到的正文内容”，不是重新访问原网页。首页和归档列表不展示“已抓取的原文”入口；搜索页的每条结果、每日摘要详情页的每条通知会展示这个入口。
...
默认情况下，搜索只匹配标题、类别、适用对象、关键词和摘要；勾选“搜索已抓取原文”后，会把 `content/raw/items/` 中对应条目的全文也纳入检索。

每日摘要详情页支持在每条通知后打开“已抓取的原文”。首页只展示当日整理后的通知内容，不展示“已抓取的原文”入口，避免首页过重。
